...
export const ProfileTopForm = ({ who, isMine, userobj }) => {
  ...
  const follow = (event) => {
    ...
    if (!onFollow.following) {
      ...
      setProfile({
        ...profile,
        follower: [userobj.uid].concat(profile.follower),
      });
    } else {
      //unFollow
      profileDispatch({
        type: "UNFOLLOWING",
        id: profile.uid,
        userNotifications: profile.notifications.filter(
          (n) =>
            n.value !== "following" ||
            n.user !== userobj.uid ||
            n.docId !== null
      ...
      setProfile({
        ...profile,
        follower: profile.follower.filter((f) => f !== userobj.uid),
      });
    }
  };
  useEffect(() => {
...
export const ProfileBottomForm = ({ isMine, userobj }) => {
  ...
  const filterTweets = tweets.filter(
    (tweet) =>
      tweet.value === "tweet" || tweet.value === "qt" || tweet.value === "rt"
  );
  const filterTweetAndAnswer = tweets.filter(
