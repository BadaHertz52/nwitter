...
export const getTweetDoc = (uid, docId) =>
  dbService.collection(`tweets_${uid}`).doc(`${docId}`).get();
export const getTweetsDocs = (uid) =>
  dbService.collection(`tweets_${uid}`).get();
//tweet
export const getTweets = (id, setFun) => {
  dbService.collection(`tweets_${id}`).onSnapshot((querysnpshot) => {
    const array = querysnpshot.docs.map((doc) => ({ ...doc.data() }));
    setFun(array.reverse());
  });
};
export const getTweet = (uid, docId, setFun) => {
  getTweetDoc(uid, docId)
    .then((doc) => {
      if (doc.exists) {
        setFun({ ...doc.data() });
      } else {
...
export const updateTweetNotifications = async (
  tweet,
  profile,
  value,
  userobj,
  ...
  const newNotification = [
    { value: value, user: userobj.uid, aboutDocId: aboutDocId },
  ];
  ...
  await dbService
    .collection(`tweets_${profile.uid}`)
    .doc(`${tweet.docId}`)
    .set(newTweet);
};
...
export const updateProfileNotification = async (user, id, notifications) => {
  const newNotification =
    notifications !== undefined ? [user].concat(notifications) : [];
  await getProfileDoc(id).set(
...
export const updateMyTweetsByMe = (
  myTweets,
  value,
  userobj,
  docId,
  ...
  myTweets.forEach((tweet) => {
    if (tweet.docId === tweet_docId) {
      ...
      myTweets[INDEX] = {
        ...tweet,
        notifications: [
          { value: value, user: userobj.uid, aboutDocId: docId },
        ].concat(tweet.notifications),
      };
      tweetDispatch({
        type: "EDIT",
        uid: userobj.uid,
...
export const deleteTweetNotification = (
  tweet,
  target_myTweet,
  userobj,
  value
) => {
  const newNotifications = tweet.notifications.filter(
    (n) =>
...
export const deleteProfileNotification = (
  ...
  const newNotifications = userProfile.notifications.filter(
    (n) =>
      n.docId !== tweet.docId ||
      n.aboutDocId !== target_myTweet.docId ||
      n.user !== userobj.uid ||
  ...
  const modifyMytweetNotifications = (tweet_docId) => {
    myTweets.forEach((tweet) => {
      if (tweet.docId === tweet_docId) {
        ...
        myTweets[INDEX] = {
          ...tweet,
          notifications: tweet.notifications.filter(
  ...
  if (tweetObj.creatorId === userobj.uid) {
    ...
    const targetMytweet =
      tweetObj.value === "tweet"
        ...
        : myTweets
            .filter((tweet) => tweet.about !== null)
            .filter(
              (tweet) =>
                tweet.about.creatorId === tweetObj.creatorId &&
                tweet.about.docId === tweetObj.docId
            )[0];
