      ...
      if(myProfile.following[0]!==undefined){
        myProfile.following.forEach(async(user)=>
        {
          await getProfileDoc(user).get().then(result=>{
            ...
            const newNotifications =profile.notifications.filter(n=> (n.value!=="following"|| n.user !== userobj.uid));
            const newFollower = profile.follower.filter(f=> f!== userobj.uid);
      ...
      if(myProfile.follower[0]!== undefined){
        myProfile.follower.forEach(async(user)=>{
          await getProfileDoc(user).get().then(
            (result) => {
              const profile= result.data();
              const newFollowing =profile.following.filter(f=> f!==userobj.uid);
              getProfileDoc(user).set({
      ...
      if(targetTweets[0]!== undefined){
        targetTweets.forEach(async(tweet)=>{
          ...
          await getTweetDoc(targetUser, tweet.about.docId).then(doc=> {
            ...
            const newNotifications= userTweet.notifications.filter(n=>n.user!== userobj.uid);
          ...
          await getProfileDoc(targetUser).get().then(async(doc)=> {
            ...
            const newNotifications= profile.notifications.filter(n=> n.user !== userobj.uid);
    ...
    return (
      ...
      <div className='delete_inner'>
        ...
        <div className='deleteBtns'>
          ...
          <button onClick={()=>{navigate('/twitter/logOut')}}>
            Log out
          </button>
        </div>
      </div>)
