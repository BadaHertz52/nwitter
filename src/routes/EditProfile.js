...
const EditProfile = ({ userobj, setIsMyProfile }) => {
  ...
  const basicPhoto =
    "https://firebasestorage.googleapis.com/v0/b/nwitter-c8556.appspot.com/o/icons8-user-64.png?alt=media&token=0e76967a-3740-4666-a169-35523d1e07cb";
  const basicHeader =
    "https://firebasestorage.googleapis.com/v0/b/nwitter-c8556.appspot.com/o/basicHeader.png?alt=media&token=3fb9d8ee-95ba-4747-a64f-65c838247ca9";
  const { myProfile, profileInput, profileDispatch } =
    useContext(ProfileContext);
  const { userName, introduce } = myProfile;

  const [header, setHeader] = useState(
    myProfile.headerUrl === "" ? basicHeader : myProfile.headerUrl
  );
  ...
  const onSubmit = async (event) => {
    ...
    profileInput.userName !== "" &&
      getProfileDoc(userobj.uid).update({ userName: profileInput.userName });
    profileInput.introduce !== "" &&
      getProfileDoc(userobj.uid).update({ introduce: profileInput.introduce });
    ...
    if (location.state !== null) {
      ...
      if (state.what !== undefined && profileInput !== undefined) {
        ...
        switch (what) {
          ...
          case "photo":
            profileInput.photoUrl !== "" &&
              setProfilePhoto(profileInput.photoUrl);
            break;
          default:
            break;
        }
    ...
    <section id="editProfile">
      ...
      <form onSubmit={onSubmit} className="back">
        ...
        <div id="editProfile_header">
          <img
            src={
              profileInput !== undefined && profileInput.headerUrl !== ""
        ...
        <div id="profile_photo">
          <img
            src={
              profileInput !== undefined && profileInput.photoUrl !== ""
                ? profileInput.photoUrl
                : photo
            }
            alt="profile"
