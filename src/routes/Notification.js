    ...
    n.value === "following"
      ...
      : navigate("/twitter/timeLine", {
          state: {
            ...
            userUid: n.user.uid,
            aboutDocId:
              n.value !== "tweet" && n.aboutTweet === "" ? null : n.aboutTweet,
          },
        });
  };
  ...
  useEffect(() => {
    const getData = async () => {
      const array = await Promise.all(
        myProfile.notifications.map((n) => {
          const result = getProfileDoc(n.user)
            .get()
            .then(async (doc) => {
              const tweet =
      ...
      const array_withTweets = await Promise.all(
        array.map((a) => {
          if (a.aboutTweet !== "") {
            const result = dbService
              .collection(`tweets_${a.user.uid}`)
              .doc(`${a.aboutTweet}`)
              .get()
              .then((doc) => ({ ...a, aboutTweet: doc.data() }));
            return result;
          } else {
            return a;
          }
    ...
    <section id="notifications" className="header">
      ...
      <div id="notificationBtns">
        ...
        <button
          ...
          onClick={() => {
            setShowAll(false);
            changeStyle(mentionBtn);
          }}
        >
          <div id="mention">Mentions</div>
        </button>
      </div>
      <div>
        {showAll
          ? notifications.map((n) =>
              n.value !== "answer" ? (
                <div class="notification" onClick={() => goTimeLine(n)}>
                  <div className="notification_left">
                    ...
                    {n.value === "tweet" && <VscBell className="tweetIcon" />}
                  </div>
